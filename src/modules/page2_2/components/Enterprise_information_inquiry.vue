  ...
  <el-col :span="6">
  	<div class="grid-content bg-purple" style="text-align:left;padding-left:50px;">
        ...
        <el-button type="primary" @click="btn_operation(button.btnCode)" v-for="button in buttons">{{button.btnName}}</el-button>
  	   <!-- <el-button type="info" @click="give_out_task">任务转移</el-button>-->
  	</div>
  </el-col>
</el-row>
...
<script>
  ...
  export default {
    ...
    methods: {
            ...
            var selelct_url=localStorage.localhost+'/wf/gotoAllotCliamTask'
...
//            console.log(res.body.data.itemStatusList);
               ...
               for(var i=0; i<res.body.data.itemStatusList.length;i++){
               	
               	  $("#select_choice").append('<option value="'+res.body.data.itemStatusList[i].dictDataCode+'">'+res.body.data.itemStatusList[i].dictDataName+'</option>');
               }
            ...
            this.criteria.unCliamOrCliamed='1';
